...
class PortfolioImagesFieldController extends GetxController {
  ...
  PortfolioImagesFieldController() {
  ...
  Future pickImage(ImageSource source, String field,
    ...
    if (imageFiles != null) {
      ...
      imageFile = imageFiles.map((imagex) => File(imagex.path)).toList();
      print("check this final ${imageFile.toString()}");

      try {
        for (int i = 0; i < imageFile.length; i++) {
          var _uuid = await _uploadRepository.image(imageFile[i], field);
          uuids.add(_uuid);
          images.add(imageFile[i]);
...
class PortfolioImagesFieldWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(PortfolioImagesFieldController(), tag: tag);
    return Container(
      child: Column(
        ...
        children: [
          Obx(() {
            return Row(
              ...
              children: [
                // buildImages(initialImages, controller.images),
