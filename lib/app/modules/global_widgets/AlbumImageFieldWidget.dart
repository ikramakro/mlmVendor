...
class AlbumImagesFieldController extends GetxController {
  ...
  Future pickImage(ImageSource source, String field,
    ...
    if (imageFile != null) {
      try {
        ...
        url.value =
            await _uploadRepository.imageReference(imageFile, "RefernceImage");
        // uuids.add(_uuid);
        images.add(imageFile);
        uploadCompleted(url.value);
        uploading.value = false;
        uploaded.value = true;
      } catch (e) {
        uploading.value = false;
...
class AlbumImagesFieldWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final controller = Get.put(AlbumImagesFieldController(), tag: tag);
    return Container(
